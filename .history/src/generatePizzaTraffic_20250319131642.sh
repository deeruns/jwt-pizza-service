...
while true; do
  response=$(curl -s -X PUT "$host/api/auth" -d '{"email":"f@jwt.com", "password":"franchisee"}' -H 'Content-Type: application/json')
  echo "Franchisee login response: $response"
  if echo "$response" | jq -e . >/dev/null 2>&1; then
    token=$(echo "$response" | jq -r '.token')
    echo "Franchisee token: $token"
  else
    token=""
    echo "Invalid JSON response for franchisee login"
  fi
  sleep 110
  [ -n "$token" ] && curl -s -X DELETE "$host/api/auth" -H "Authorization: Bearer $token" > /dev/null
  echo "Logging out franchisee..."
  sleep 10
done &
pid3=$!
...
while true; do
  response=$(curl -s -X PUT "$host/api/auth" -d '{"email":"d@jwt.com", "password":"diner"}' -H 'Content-Type: application/json')
  echo "Diner login response: $response"
  if echo "$response" | jq -e . >/dev/null 2>&1; then
    token=$(echo "$response" | jq -r '.token')
    echo "Diner token: $token"
  else
    token=""
    echo "Invalid JSON response for diner login"
  fi
  [ -n "$token" ] && curl -s -X POST "$host/api/order" -H 'Content-Type: application/json' -d '{"franchiseId": 1, "storeId":1, "items":[{"menuId": 1, "description": "Veggie", "price": 0.05}]}' -H "Authorization: Bearer $token" > /dev/null
  echo "Bought a pizza..."
  sleep 20
  [ -n "$token" ] && curl -s -X DELETE "$host/api/auth" -H "Authorization: Bearer $token" > /dev/null
  echo "Logging out diner..."
