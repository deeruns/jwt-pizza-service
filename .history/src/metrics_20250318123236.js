...
// Metric sending function to Grafana
...
//     .then((response) => {
//       if (!response.ok) {
//         console.error(`Failed to push ${metricName}: ${response.status}`);
//       } else {
...
function requestTracker(req, res, next) {
  ...
  res.on('finish', () => {
    ...
    sendMetricToGrafana('http_requests_total', 1, {
      method,
      status_code: statusCode,
      route,
    });
    sendMetricToGrafana('http_request_duration_ms', duration, {
      method,
      route,
    });
  });
...
function getCpuUsagePercentage() {
  ...
  return (cpuUsage * 100).toFixed(2); // Percentage
}
...
function getMemoryUsagePercentage() {
  ...
  return ((usedMemory / totalMemory) * 100).toFixed(2); // Percentage
}
...
function sendMetricsPeriodically(period) {
  setInterval(() => {
    try {
      // HTTP metrics are handled by middleware; add others here as counters accumulate

      // System metrics
      const cpuUsage = getCpuUsagePercentage();
      const memoryUsage = getMemoryUsagePercentage();
      sendMetricToGrafana('cpu_usage_percentage', cpuUsage);
      sendMetricToGrafana('memory_usage_percentage', memoryUsage);

      // Placeholder for additional metrics (to be implemented)
      // authMetrics();
      // userMetrics();
      // purchaseMetrics();
    } catch (error) {
...
// Start periodic reporting (e.g., every 10 seconds)
